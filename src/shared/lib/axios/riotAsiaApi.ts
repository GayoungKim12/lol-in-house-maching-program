...
// 라이엇 API 인스턴스 생성
const riotAsiaApi = axios.create({
  baseURL: `${import.meta.env.VITE_RIOT_ASIA_API_URL || 'https://asia.api.riotgames.com'}`,
  headers: {
...
// 응답 에러 처리를 위한 인터셉터
riotAsiaApi.interceptors.response.use(
  (response) => response,
  (error) => {
